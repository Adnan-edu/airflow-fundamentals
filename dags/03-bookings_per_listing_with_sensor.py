...
def bookings_spark_pipeline():
    ...
    # The FileSensor below waits for the listings file for the current month to appear in the local filesystem.
    # - task_id: Unique identifier for this sensor task in the DAG.
    # - fs_conn_id: Airflow connection ID for the local filesystem.
    # - filepath: The path to the listings file, templated with the current execution date's year and month.
    # - poke_interval: The sensor checks for the file every 30 seconds.
    # - timeout: If the file does not appear within 600 seconds (10 minutes), the sensor task will fail and the DAG run will be marked as failed.
